...
{
    /// <summary>
    /// Scaled time that can be effected by the global time scale.
    /// For example, if we set Unity's time scale to be 2, our game would run twice as fast.
    /// Unscaled time is that time since the game has started, but scaled by this value.
    /// </summary>
    SCALEDTIME,
    /// <summary>
    ...
    /// give us values as if teh time were 1.
    /// </summary>
    UNSCALEDTIME,
    /// <summary>
...
public class TimerBehaviour : MonoBehaviour
{
    ...
    public static TimerBehaviour Instance
    {
        get
        {
            if (!_instance)
            {
                _instance = FindObjectOfType<TimerBehaviour>();
            }

            if (!_instance)
            {
                GameObject timer = new GameObject("TimerObject");
                _instance = timer.AddComponent<TimerBehaviour>();
            }
    ...
    /// <summary>
    /// Calls the given event after the given amount of time of frames have passed.
    /// </summary>
    ...
    /// <summary>
    /// Stopes the given timed action; preventing the event from being called.
    /// </summary>
